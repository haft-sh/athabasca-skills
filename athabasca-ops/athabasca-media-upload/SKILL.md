...
```json
{
  ...
  "asset": {
    ...
    "sha256": "...",
    "storageProvider": "r2",
    "storageKey": "<project-slug>/research/img_xxx.jpg",
    "publicUrl": "https://media.example.com/<project-slug>/research/img_xxx.jpg",
    "provenanceNote": "..."
  }
}
```
...
- first inspect `$HOME/.hermes/image_cache/` for the other recent sibling `.jpg` files from the same send batch
...
curl -sS -X POST http://localhost:3000/api/projects/<project-slug>/media \
  -F file=@/tmp/<project-slug>-claude-response.md \
  -F phase=research \
  -F category=research \
  ...
  -F 'metadataJson={"workflow":"multi-model-text-comparison","artifactType":"model_response_markdown","contextLabel":"research comparison","sourcePath":"/tmp/<project-slug>-claude-response.md"}' \
