# Absorbed skill: `comfyui`

This file preserves the original SKILL.md content from `comfyui` after consolidation into the umbrella skill `creative-production-workflows`. Relative support-file links have been rewritten to their re-homed umbrella paths where they were explicit.
...
prerequisites:
  commands: ["python3"]
setup:
  help: "Run scripts/comfyui-hardware_check.py FIRST to decide local vs Comfy Cloud; then scripts/comfyui-comfyui_setup.sh auto-installs locally (or use Cloud API key for platform.comfy.org)."
metadata:
  hermes:
    tags:
      - comfyui
...
`references/comfyui-workflows/README.md`.
...
python3 scripts/comfyui-hardware_check.py
```
...
```bash
python3 scripts/comfyui-health_check.py
...
```bash
python3 scripts/comfyui-extract_schema.py workflow_api.json --summary-only
# → {"parameter_count": 12, "has_negative_prompt": true, "has_seed": true, ...}

python3 scripts/comfyui-extract_schema.py workflow_api.json
...
python3 scripts/comfyui-run_workflow.py \
...
python3 scripts/comfyui-run_batch.py \
  --workflow sdxl.json \
...
| "install ComfyUI" | comfy-cli | `bash scripts/comfyui-comfyui_setup.sh` |
...
```bash
python3 scripts/comfyui-hardware_check.py --json
# Optional: also probe `torch` for actual CUDA/MPS:
python3 scripts/comfyui-hardware_check.py --json --check-pytorch
```
...
```bash
bash scripts/comfyui-comfyui_setup.sh
# Or with overrides:
bash scripts/comfyui-comfyui_setup.sh --m-series --port=8190 --workspace=/data/comfy
```
...
4. Run workflows:
   ```bash
   python3 scripts/comfyui-run_workflow.py \
     --workflow references/comfyui-workflows/flux_dev_txt2img.json \
     --args '{"prompt": "..."}' \
     --host https://cloud.comfy.org \
     --output-dir ./outputs
   ```
...
python3 scripts/comfyui-check_deps.py my_workflow.json
# → are this workflow's nodes/models/embeddings installed?

python3 scripts/comfyui-run_workflow.py \
  --workflow references/comfyui-workflows/sd15_txt2img.json \
...
```bash
python3 scripts/comfyui-run_workflow.py \
  --workflow references/comfyui-workflows/sdxl_img2img.json \
  --input-image image=./photo.png \
...
```bash
python3 scripts/comfyui-run_workflow.py \
  --workflow references/comfyui-workflows/sdxl_inpaint.json \
  --input-image image=./photo.png \
...
python3 scripts/comfyui-fetch_logs.py --tail-queue --host https://cloud.comfy.org
```
...
Use `python3 scripts/comfyui-health_check.py` to run the whole list at once. Manual:
