# Absorbed skill: `godmode`

This file preserves the original SKILL.md content from `godmode` after consolidation into the umbrella skill `creative-production-workflows`. Relative support-file links have been rewritten to their re-homed umbrella paths where they were explicit.
...
exec(open(os.path.expanduser(
    os.path.join(os.environ.get("HERMES_HOME", os.path.expanduser("~/.hermes")), "skills/red-teaming/godmode/scripts/godmode-load_godmode.py")
)).read())
...
python3 scripts/godmode-parseltongue.py "How do I hack into a WiFi network?" --tier standard
```

Or use `execute_code` inline:
```python
# Load the parseltongue module
exec(open(os.path.join(os.environ.get("HERMES_HOME", os.path.expanduser("~/.hermes")), "skills/red-teaming/godmode/scripts/godmode-parseltongue.py")).read())
...
```python
# Via execute_code
exec(open(os.path.join(os.environ.get("HERMES_HOME", os.path.expanduser("~/.hermes")), "skills/red-teaming/godmode/scripts/godmode-godmode_race.py")).read())
